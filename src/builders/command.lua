...
command = Builder {
	...
	prepare_input = function (_, b)
		if b.run and utils.getOption 'target' ~= 'run' then
			b.cmd = ''
		end
		return ''
	end
...
NOTE: if this command build is a "run" target, set the "run" field to true,
so that it'll only be executed on the "run" target]]
